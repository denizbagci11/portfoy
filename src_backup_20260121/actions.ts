...
export async function getAssetSettings(targetUserId?: string) {
    try {
        ...
        // Also fetch data sources to include 'dataSource' field
        ...
        // 1. Process Settings
        // 1. Process Settings
        settings.forEach((curr: any) => {
            combined[curr.asset] = {
                driver: curr.driver,
                manualPrice: curr.manualPrice,
                priceCurrency: curr.priceCurrency,
                lastFetchedAt: curr.lastFetchedAt // Return timestamp to frontend
            };
        });

        // 2. Merge Data Sources
        sources.forEach((curr: any) => {
            if (!combined[curr.asset]) combined[curr.asset] = { driver: 'USD' }; // Default if only source exists
            combined[curr.asset].dataSource = curr.source;
            combined[curr.asset].ticker = curr.ticker;
        });

        return combined;
    } catch (err) {
        console.error("Error getting asset settings:", err)
        return {}
    }
}
...
export async function updateAssetPriceAction(asset: string, price: number, currency: string) {
    try {
        ...
        await (prisma.assetSettings as any).upsert({
            ...
            update: { manualPrice: price, priceCurrency: currency, lastFetchedAt: new Date() },
            create: { asset, driver: 'USD', manualPrice: price, priceCurrency: currency, lastFetchedAt: new Date(), userId: session.user.id }
        })
...
export async function fetchYahooPriceAction(ticker: string) {
    try {
        ...
        if (price !== null) {
            // Special handling for precious metals to convert Ounce to Gram
            if (['PL=F', 'PF=F'].includes(ticker)) {
                // Platinum & Palladium: Custom conversion factor
                price = price / 31.10
            } else if (['GC=F', 'SI=F', 'PA=F'].includes(ticker)) {
                // Other precious metals: Standard troy ounce conversion
                price = price / 31.1035
            }
            return { success: true, price }
        }
                ...
                if (result && result.price > 0) {
                    ...
                    await (prisma as any).assetSettings.upsert({
                        ...
                        update: {
                            ...
                            // driver: result.currency === 'USD' ? 'USD' : 'TRY', // REMOVED: Don't overwrite user preference
...
export async function getMonthlyHistory() {
    try {
        const session = await auth()
        console.log('[getMonthlyHistory] Session:', session?.user ? 'exists' : 'missing');

        if (!session?.user) {
            console.log("[getMonthlyHistory] No session found");
            return []
        }

        let userId = session.user.id;

        // Fallback: If ID is missing (stale session), try to find by name OR username
        if (!userId) {
            console.log(`[getMonthlyHistory] Missing ID for session user:`, session.user);
            const dbUser = await prisma.user.findFirst({
                where: {
                    OR: [
                        { username: session.user.name || '' },
            ...
            if (dbUser) {
                userId = dbUser.id;
                console.log(`[getMonthlyHistory] Recovered user from DB: ${dbUser.username} (ID: ${userId})`);
            } else {
                console.log('[getMonthlyHistory] Could not recover user ID');
                return [];
            }
        }

        console.log(`[getMonthlyHistory] Fetching history for user ID: ${userId}`);
        ...
        console.log(`[getMonthlyHistory] Found ${history.length} history records`);
...
export async function saveMonthlyHistory(dateStr: string, totalTRY: number, totalUSD: number) {
    try {
        const session = await auth()
        if (!session?.user?.id) return { success: false }

        const date = new Date(dateStr);
        // Ensure it is set to the correct date in UTC/Local as consistent with storage
        // We expect dateStr to be "YYYY-MM-DD" of the month end.

        await prisma.monthlyHistory.upsert({
            where: {
                date_userId: {
                    date: date,
                    userId: session.user.id
        ...
        console.error("Error saving monthly history:", err)
        return { success: false }
    }
}
