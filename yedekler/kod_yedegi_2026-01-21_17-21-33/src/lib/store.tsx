        ...
        const loadFromDB = async () => {
            ...
            if (status === 'authenticated') {
                ...
                try {
                    ...
                    // Auto-fetch Yahoo Finance prices logic removed to prevent unwanted updates on Analysis page.
                    // Price updates are now handled exclusively by DashboardStats component.
                } catch (err) {
...
export function PortfolioProvider({ children }: { children: React.ReactNode }) {
    ...
    const updateAssetPrice = async (asset: string, price: number, currency: string) => {
        ...
        setAssetSettings(prev => ({
            ...prev,
            [asset]: { ...(prev[asset] || { driver: 'USD' }), manualPrice: price, priceCurrency: currency, lastFetchedAt: new Date() }
        }));

        const result = await updateAssetPriceAction(asset, price, currency);
        if (!result.success) {
            console.error('[updateAssetPrice] Failed:', result.error);
            setAssetSettings(prev => ({ ...prev, [asset]: original }));
            alert("Fiyat güncellenemedi! Detay için console'a bakın (F12).");
        }
    };
